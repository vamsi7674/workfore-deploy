...
import { AdminPerformanceService } from '../../../services/admin-performance.service';
...
export class PerformanceReviews implements OnInit {
    ...
    loadReviews(): void {
        ...
        this.error.set('');
        this.adminPerformanceService.getAllReviews(
    ...
    openDetailModal(review: PerformanceReview): void {
    ...
    toggleFeedbackForm(): void {
        ...
        this.showFeedbackForm.set(!this.showFeedbackForm());
    }
    ...
    submitFeedback(): void {
        ...
        this.adminPerformanceService.provideReviewFeedback(review.reviewId, request).subscribe({
            next: (res) => {
                if (res.success) {
    ...
    getRatingColor(rating: number | null): string {
        if (!rating) return 'text-slate-600';
    ...
    getEmployeeDepartment(review: PerformanceReview): string {
        if (review.employee && typeof review.employee === 'object') {
            if (review.employee.department && review.employee.department.departmentName) {
                return review.employee.department.departmentName;
            }
        }
        return '';
    }
